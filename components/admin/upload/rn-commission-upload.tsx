...
export function RNCommissionUpload() {
  ...
  const parseExcelData = (data: string) => {
    ...
    expectedColumns.forEach(col => {
      const headerIndex = headers.findIndex(h =>
        ...
        (col.key === 'rn_premium_php' && h.includes('premium')) ||
        (col.key === 'rn_commission_php' && h.includes('commission'))
      );
      if (headerIndex !== -1) {
    ...
    dataRows.forEach((row, index) => {
      ...
      if (columnMapping.rn_premium_php !== undefined) {
        ...
        if (value) {
          ...
          if (isNaN(numValue) || numValue < 0) {
            rowErrors.push('RN Premium must be a positive number');
          } else {
      ...
      if (columnMapping.rn_commission_php !== undefined) {
        ...
        if (value) {
          ...
          if (isNaN(numValue) || numValue < 0) {
            rowErrors.push('RN Commission must be a positive number');
          } else {
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      console.log('Uploading RN commission data:', parsedData);
