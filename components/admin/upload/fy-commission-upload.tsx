...
export function FYCommissionUpload() {
  ...
  const parseExcelData = (data: string) => {
    ...
    dataRows.forEach((row, index) => {
      ...
      // Parse fy_premium_php (currency format)
      if (columnMapping.fy_premium_php !== undefined) {
        ...
        if (value) {
          ...
          if (numValue === null || numValue < 0) {
            rowErrors.push('FY Premium (PHP) must be a positive number (currency format accepted)');
          } else {
      ...
      // Parse fy_commission_php (currency format)
      if (columnMapping.fy_commission_php !== undefined) {
        const value = cells[columnMapping.fy_commission_php]?.trim();
        if (value) {
          const numValue = parseCurrencyString(value);
          if (numValue === null || numValue < 0) {
            rowErrors.push('FY Commission (PHP) must be a positive number (currency format accepted)');
          } else {
