-- Fix duplicate detection PostgreSQL record field access issue
-- Temporarily disable duplicate detection to get uploads working
-- We'll add it back properly after core functionality is restored
...
BEGIN
  ...
  -- Simple bulk insert using jsonb_to_recordset without duplicate detection for now
  BEGIN
    CASE p_table_name
