-- Hard reset: Revert to the ORIGINAL working RPC function
-- This was working perfectly before we started all the "fixes"

DROP FUNCTION IF EXISTS upload_with_deduplication(text, jsonb, text[]);
DROP FUNCTION IF EXISTS simple_bulk_insert(text, jsonb);
DROP FUNCTION IF EXISTS debug_insert_test(jsonb);

-- Restore the original working function (before all our changes)
