-- Fix upload_with_deduplication RPC function to properly handle system columns
-- The function was trying to access updated_at/created_at from input records which don't have them
...
BEGIN
  ...
  IF p_records IS NULL OR jsonb_array_length(p_records) = 0 THEN
    RETURN jsonb_build_object(
      ...
      'duplicates_removed', 0,
      'records_inserted', 0,
      'errors', ARRAY['No valid records to insert']
    );
  END IF;
  ...
  IF insert_columns IS NULL OR array_length(insert_columns, 1) = 0 THEN
    RETURN jsonb_build_object(
      ...
      'duplicates_removed', 0,
      'records_inserted', 0,
      'errors', ARRAY['No valid columns found in records']
    );
  END IF;
  ...
  FOR i IN 0..jsonb_array_length(p_records)-1 LOOP
    ...
    -- Prepare values for this record (only from input columns, not system columns)
