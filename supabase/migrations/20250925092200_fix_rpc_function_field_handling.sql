...
BEGIN
  ...
  -- Get valid column names from the table schema (excluding system columns)
  SELECT array_agg(column_name ORDER BY column_name) INTO temp_columns
  FROM information_schema.columns
  WHERE table_schema = 'public'
    AND table_name = p_table_name
    AND column_name NOT IN ('id', 'created_at', 'updated_at');

  valid_columns := temp_columns;

  -- Get column names from the first record that match valid table columns
  ...
  IF insert_columns IS NULL OR array_length(insert_columns, 1) = 0 THEN
    RETURN jsonb_build_object(
      'success', false,
      'duplicates_removed', 0,
      'records_inserted', 0,
      'errors', ARRAY['No valid columns found in data. Valid columns for ' || p_table_name || ' are: ' || array_to_string(valid_columns, ', ')]
    );
  END IF;
  ...
  FOR i IN 0..jsonb_array_length(p_records)-1 LOOP
    ...
    -- Build WHERE conditions for duplicate detection using only valid duplicate fields
    ...
    -- Prepare values for this record using only valid columns
